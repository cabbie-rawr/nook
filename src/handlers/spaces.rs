...
pub async fn dashboard(
    ...
    let active = sqlx::query_as::<_, Space>(
        "SELECT * FROM spaces WHERE user_id = $1 AND archived_at IS NULL ORDER BY created_at DESC",
    )
    .bind(user.id)
    ...
    let archived = sqlx::query_as::<_, Space>(
        "SELECT * FROM spaces WHERE user_id = $1 AND archived_at IS NOT NULL ORDER BY archived_at DESC",
    )
    .bind(user.id)
    .fetch_all(&state.pool)
    .await?;
...
pub async fn create_space(
    ...
    sqlx::query("INSERT INTO spaces (user_id, name, color) VALUES ($1, $2, $3)")
        .bind(user.id)
        .bind(name)
        .bind(form.color)
        .execute(&state.pool)
...
async fn owned_space(pool: &PgPool, space_id: i64, user_id: uuid::Uuid) -> Result<Space, AppError> {
    sqlx::query_as::<_, Space>("SELECT * FROM spaces WHERE id = $1 AND user_id = $2")
        .bind(space_id)
        .bind(user_id)
        .fetch_optional(pool)
        .await?
...
pub async fn space_detail(
    ...
    let tasks = sqlx::query_as::<_, Task>(
        r#"SELECT * FROM tasks WHERE space_id = $1
...
pub async fn edit_space(
    ...
    sqlx::query("UPDATE spaces SET name = $1, color = $2 WHERE id = $3 AND user_id = $4")
        .bind(name)
        .bind(form.color)
        .bind(space_id)
        .bind(user.id)
...
pub async fn archive_space(
    ...
    if space.archived_at.is_some() {
        sqlx::query("UPDATE spaces SET archived_at = NULL WHERE id = $1")
            .bind(space_id)
            .execute(&state.pool)
            .await?;
    } else {
        sqlx::query("UPDATE spaces SET archived_at = $1 WHERE id = $2")
            .bind(chrono::Utc::now())
            .bind(space_id)
            .execute(&state.pool)
            .await?;
...
pub async fn delete_space(
    ...
    let result = sqlx::query("DELETE FROM spaces WHERE id = $1 AND user_id = $2")
        .bind(space_id)
        .bind(user.id)
        .execute(&state.pool)
        .await?;
